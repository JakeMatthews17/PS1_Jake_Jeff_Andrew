...
**Answer:**
```{r}
reg <- lm(out$Y~out$V1+out$V2)
summary(reg, digits=3)
```

We would say our estimates of beta_1 and beta_2 are unbiased as they are both very close to our initial assumptions of 3 and 6 respectively. This regression also has an R^2 value of .9865, meaning it explains about 98% of the variance of the population.
...
**Answer:**
```{r}
reg2 <- lm(out$Y~out$V1)
summary(reg2, digits=3)
```

In this regression, where we omit one of the variables, the coefficient for beta_1 jumps to 7. This is not close to our assumption of beta_1 so we think it may be biased. The R^2 for this model is .8167, which means it explains less of the population variance and is likley biased in comparison to the first model.
...
**Answer:**
```{r}
out$Yadj <- out$Y-B2*out$V2
reg3 <- lm(out$Yadj~out$V1)
summary(reg3)
```
We don't think beta_1 is biased in this instance because of how close it is to our asumption of 3 and the increased value of R^2 for this model (.8966). This lack of bias is because Y adjusted is a function of beta_2, therefore not omitting it in the same way as the previous model.
...
**Answer:**
```{r}
plot <- ggplot(out, aes(V1, y=value))+geom_point(aes(y=Y, col="Y", alpha=0.05),color="black")+geom_point(aes(y=Yadj,col="Yadj", alpha=0.05), color="red")+geom_smooth(method=`lm`,aes(y=Y,col="Y", alpha=0.1),color="black")+geom_smooth(method=`lm`,aes(y=Yadj,col="Y",alpha=0.1),color="red")+theme_bw()+labs(x="V1 value", y="Y value/Y_adj value", title="Relationship between Y and V1, Y_adj and V1")
plot
```

We changed the color of one the variables to distinguish them from each other, added labels for the x and y axis, added a new title,changed the opacity of the data making it easier to see all the points, and changed the color of the regression line.
...
library(readr)
cps_clean_1_<-read_csv("cps_clean.csv")
cps_clean_1_ <- cps_clean_1_
reg_cps <- lm(formula=inctot~edu, data=cps_clean_1_)
summary(reg_cps)
```
...
**Answer:**
```{r}
reg_cps2 <- lm(formula=inctot~edu+female,data=cps_clean_1_)
summary(reg_cps2)
```

After we added a dummy variable for gender our beta_1 estimate for education increased slightly while constraining the standard error. Overall we found this change to not be very significant, and our model may benefit from additional interaction variables.
